...
void printlevel(int current_node,vector<vector<int>>&adj,int parent,vector<int>&ans,vector<int>&level,int current_level){
	...
	for(int neighbour:adj[current_node]){
		 if(neighbour!=parent){
		 	printlevel(neighbour,adj,current_node,ans,level,current_level+1);
		 }
	}
...
void solve(){
	...
	printlevel(0,adj,-2,dfs_traversal,level,0);
	
	
	VPL arr;
	rep(i,0,n){
		arr.push_back({i,level[i]});
	}
	
	rep(i,0,sz(arr)){
		cout<<"level of node "<<arr[i].F<<" is "<<arr[i].S<<"\n";
	}
