...
//<--------------------FINDING LCA IN 0(LOG(N)) WITHOUT DOING BINARY SEARCH ON LAST FALSE WHEN BOTH A AND B DON'T SHARE SAME ANCESTOR--------------------------------------->


//already know parent of each node in the powers of 2(using kth_parent of node), like 
/*
2^0th parent
2^1st parent
2^2nd parent
...
//<--------------------FINDING LCA USING IN_TIME OUT_TIME TRICK(0(LOG(N))) WITHOUT BRINGING BOTH NODES AT SAME LEVEL--------------------------------------->

/*
is kth parent of a is ancestor of node b or not

going up in node a, such that the kth_parent of node 'a' will be the ancestor of node 'b'


*/


//this approach works without making both nodes a and b to same level

void dfs_in_time_out_time(int current_node,int parent,VVL&adj,VL&in_time,VL&out_time,int&global_timer){
	
	in_time[current_node]=global_timer++;
	for(int neighbour:adj[current_node]){
		if(neighbour!=parent){
			dfs_in_time_out_time(neighbour,current_node,adj,in_time,out_time,global_timer);
		}
	}
	
	out_time[current_node]=global_timer++;
}

//x is ancestor of y
bool is_ancestor(int x,int y,VL&in_time,VL&out_time){
...
/*
finding lca(a,b) without bringing a and b to same level


first approach->

find out last parent of node 'a' which is not ancestor of node 'b'


second approach->

find out last parent of node 'b' which is not ancestor of node 'a'
...
1) is node 'a' is ancestor of node 'b'	-> return a
2) is node 'b' is ancestor of node 'a'	-> return b
3) find last parent of node 'a' which is not ancestor of node 'b'
and return the first parent of that node 

*/


int lca_in_time_out_time(int a,int b,VL&in_time,VL&out_time,VVL&parents){
	
	
	int n=sz(parents[0]);	//number of nodes of each node are being just next immediate neighbour of the present node
	...
	if(is_ancestor(b,a,in_time,out_time)){
		return b;
	}
	
	
	//finding the last ancestor using power of 2 which is just less than n
	
	for(int k=log2(n);k>=0;k--){
		int parent_a=parents[k][a];
		
		if(!is_ancestor(parent_a,b,in_time,out_time)){
			a=parent_a;
		}
	}
...
void solve(){
	...
	for(int k=1;k<=(log2(n));k++){
 	...
 	VL out_time(n,0);
  	...
  	dfs_in_time_out_time(0,-2,adj,in_time,out_time,global_timer);
