...
void numberOfSubtreeOfEachNode(int current_node,VVL&adj,int parent,VL&subtree){

	for(int neighbour:adj[current_node]){
		 if(neighbour!=parent){
		 	
		 	numberOfSubtreeOfEachNode(neighbour,adj,current_node,subtree);
            subtree[current_node]+=subtree[neighbour];
		 }
	}
	
	subtree[current_node]+=1;
	
}


//printing the number of children of each node
...
void solve(){
	...
	VL subtree(n,0);
    int leaf_cnt=0;
    numberOfSubtreeOfEachNode(0,adj,-2,subtree);
    rep(i,0,n){
        // cout<<i<<" "<<subtree[i]<<"\n";
    //    cout<<"for node "<<i<<"the number of subtree size is "<<subtree[i]<<"\n";
    ...
    rep(i,0,n){
      ...
      cout<<"the count of leaf nodes are "<<leaf_cnt<<"\n";


	// dfs(0,adj,-2,dfs_traversal);
// 	
	// rep(i,0,sz(dfs_traversal)){
...
//<---------------------------------------------------------------------------------------->
	...
	*/
     ...
     3) just counting the count of children each time dfs it called for current_node

     4) for counting the number of current_node, increment the count of node of neighbour 
     and in final to include that current_node itself, increment the count by 1

     for counting the number of leaf_nodes,the size of the subtree should be 1
