...
void numberOfChildrenOfEachNode(int current_node,VVL&adj,int parent,VL&children){
	...
	for(int neighbour:adj[current_node]){
		 if(neighbour!=parent){
		 	...
		 	numberOfChildrenOfEachNode(neighbour,adj,current_node,children);
		 }
	}
...
void solve(){
	...
	VL children(n,0);
	numberOfChildrenOfEachNode(0,adj,-2,children);
	rep(i,0,n){
		cout<<i<<" "<<children[i]<<"\n";
        // cout<<"for node : "<<i<<" number of children are"<<children[i]<<"\n";
        /*
        or it can be
        for root node=size of adjacency list
        for non-rooted node=size of adjacency list-1
        */
	}
