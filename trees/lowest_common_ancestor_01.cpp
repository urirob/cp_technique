...
void dfs(int current_node,int parent,VL&first_parent,VVL&adj){
	first_parent[current_node]=parent;
	
	for(int neighbour:adj[current_node]){
		if(neighbour!=parent){
			dfs(neighbour,current_node,first_parent,adj);
		}
	}
} 
...
int lca(int a,int b,VL&distance,VVL&parents){
