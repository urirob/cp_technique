...
//<------------------------------------------------------------------------------------------> 	
  	...
  	for(int bimask=(1<<n)-1;bitmask>=1;bitmask--){
  		//for last element to be present in bitmask, every element need to be present in the final bitmask
  		//iterate over all the values of the last
  		for(int last=0;last<n;last++){
  			
	  			if(bitmask&&(1<<last)==0){
	  				//if the bit is not set in the bitmask,then it is not valid state 
	  				continue;
	  			}
//<------------------------------------------------------------------------------------------> 	  			
  			//try to find dp[last][bitmask] here
  		...
  		for(int i=0;i<n;i++){
			...
			int new_last=i;
			int new_bitmask=(bitmask|(1<<i));
			double ans_here=dist(points[i],points[last]) + dp[new_last][new_bitmask];
